...
const getHashForFilePath = require('./utils/getHashForFilePath');
...
class AwsProvider {
  ...
  async getServiceArtifactS3Path(artifactFilePath) {
    const { deploymentDirectoryPrefix } = this.serverless.service.package;
    const hash = await getHashForFilePath(artifactFilePath, 'hex');
    return `${deploymentDirectoryPrefix}/code-artifacts/${hash}.zip`;
  }

  async mapServiceArtifactS3Path(artifactFilePath) {
    const { service } = this.serverless;
    if (!service.package.artifactsMap) service.package.artifactsMap = {};

    if (service.package.artifactsMap[artifactFilePath]) {
      return service.package.artifactsMap[artifactFilePath];
    }
    const artifactS3Path = await this.getServiceArtifactS3Path(artifactFilePath);
    service.package.artifactsMap[artifactFilePath] = artifactS3Path;
    return artifactS3Path;
  }
}
